...
export default function PracticeHistoryReview() {
  const navigate =
    useNavigate();
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8080/api/exam/review/${sessionId}`
    )
      .then((res) => {
        if (!res.ok) {
          throw new Error(
  ...
  const getFeedback =
    () => {
      ...
      if (
        ...
        feedback.push(
          "👍 Khá tốt! Còn một số lỗ hổng nhỏ."
        );
      } else if (
        stats.score >= 40
      ) {
        ...
        feedback.push(
          "⚠️ Hãy xem kỹ phần giải thích bên dưới."
        );
      }
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <button
              ...
              className="px-5 py-3 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 transition shadow"
            >
              ← Quay lại
            </button>
          </div>
...
function Card({
  ...
  return (
    ...
    >
      ...
      <div
        className={`text-4xl font-extrabold mt-2 ${text}`}
      >
        {value}
      </div>
    </div>
...
function QuestionCard({
  ...
  return (
    <div
      className={`rounded-3xl bg-white shadow-lg p-6 border-l-8 ${
        q.isCorrect
          ? "border-green-500"
          : "border-red-500"
      }`}
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 text-xs bg-slate-200 inline-block px-3 py-1 rounded-full">
            Part{" "}
            {q.part}
            {" • "}
            {q.label?.replaceAll(
